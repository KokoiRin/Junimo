...
- [P1][调试/验证] 手动截图采集已能统计今天文件；后续如仍出现“截图内容无效”，再增加图片内容有效性检测或分类，而不是恢复后台权限链路。

## Done

- [P0][Bug 修复] 修复 Codex 实时配额连接：`account/rateLimits/read` 在 Codex.app 0.142.x 上可能接近 25 秒才返回，短连接 adapter 改为等待目标响应到 35 秒并强制收口子进程，避免误显示“实时配额未连接”。
