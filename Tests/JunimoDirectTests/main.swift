...
// 业务语义：release tag 可以带 v 前缀，版本比较必须按数字段判断新旧。
let parsedReleaseVersion = ReleaseVersion(tag: "v0.1.5")
expect(parsedReleaseVersion?.description == "0.1.5", "ReleaseVersion should normalize v-prefixed tags")
expect(ReleaseVersion("0.1.5")! > ReleaseVersion("0.1.4")!, "Newer release version should compare greater")
expect(ReleaseVersion("0.1.5")! == ReleaseVersion("0.1.5")!, "Equal release versions should compare equal")
expect(ReleaseVersion("0.1.4")! < ReleaseVersion("0.1.5")!, "Older release version should compare lower")
expect(ReleaseVersion(tag: "latest") == nil, "Unparseable release tag should fail parsing")

let updateNow = Date(timeIntervalSince1970: 110)
var selfUpdateFeature = SelfUpdateFeature(currentVersion: ReleaseVersion("0.1.4")!)
selfUpdateFeature.startChecking(now: updateNow)
expect(selfUpdateFeature.snapshot.state == .checking, "SelfUpdateFeature should expose checking state")
selfUpdateFeature.applyReleaseCheck(.success(ReleaseInfo(version: ReleaseVersion("0.1.5")!, assetName: "Junimo-macos-arm64.zip")), now: updateNow)
expect(selfUpdateFeature.snapshot.state == .updateAvailable, "Newer release should become available")
expect(selfUpdateFeature.snapshot.latestVersion == ReleaseVersion("0.1.5")!, "Available update should keep latest version")
selfUpdateFeature.startInstalling(now: updateNow)
expect(selfUpdateFeature.snapshot.state == .installing, "Available update should move to installing after user intent")
selfUpdateFeature.applyInstallFailure(message: "failed to launch updater", now: updateNow)
expect(selfUpdateFeature.snapshot.state == .installFailed, "Install launch failure should be visible")

var upToDateFeature = SelfUpdateFeature(currentVersion: ReleaseVersion("0.1.5")!)
upToDateFeature.applyReleaseCheck(.success(ReleaseInfo(version: ReleaseVersion("0.1.5")!, assetName: "Junimo-macos-arm64.zip")), now: updateNow)
expect(upToDateFeature.snapshot.state == .upToDate, "Equal release should be up to date")
upToDateFeature.startInstalling(now: updateNow)
expect(upToDateFeature.snapshot.state == .upToDate, "Up-to-date state should not enter installing")
upToDateFeature.applyReleaseCheck(.failure("bad release tag"), now: updateNow)
...
expect(actionCoordinator.selfUpdateSnapshot.currentVersion == ReleaseVersion("0.1.11")!, "Coordinator default self-update version should match this release")
...
expect(oldVersionCoordinator.selfUpdateSnapshot.state == .checking, "Coordinator should forward self-update check intent")
oldVersionCoordinator.applySelfUpdateCheck(.success(ReleaseInfo(version: ReleaseVersion("0.1.5")!, assetName: "Junimo-macos-arm64.zip")), now: updateNow)
expect(oldVersionCoordinator.selfUpdateSnapshot.state == .updateAvailable, "Coordinator should expose available update")
oldVersionCoordinator.startSelfUpdateInstall(now: updateNow)
expect(oldVersionCoordinator.selfUpdateSnapshot.state == .installing, "Coordinator should forward install intent only after update is available")
